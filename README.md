...
# 3. Prepare the project (copy the template, fill placeholders):
#    cp -r kubernetes/template kubernetes/panservice-servizi   # untracked (gitignored)
#    then set namespace + __IMAGE_TAG__ in overlays/<env>/kustomization.yaml and
#    __INGRESS_HOST__ / __CLUSTER_ISSUER__ in overlays/<env>/ingress.yaml
#    (see kubernetes/template/README.md). panservice-servizi already exists on disk.

# 4. Deploy: Secret (out-of-band) + the overlay — or just ./scripts/deploy.sh
kubectl apply -f kubernetes/secret.yaml -n magikup
kubectl apply -k kubernetes/panservice-servizi/overlays/prod
...
### Manifest layout (kustomize base + overlay, per project)

Every project under `kubernetes/` is a kustomize **`base/`** (cluster-agnostic workload: ServiceAccount, Deployment, PVCs) plus **`overlays/<env>/`** (the real per-cluster values: namespace, image tag, ConfigMap, Service/Ingress, StorageClass). Deploy an overlay with `kubectl apply -k kubernetes/<project>/overlays/<env>`.

Only the **`template/`** project is committed — a blanked skeleton. The real projects live on disk, **untracked** (their folders are gitignored).

| Project (overlay) | Tracked? | Cluster specifics |
|------|----------|----------|
| `template/overlays/example` | ✅ committed | Blanked skeleton with placeholders `__IMAGE_TAG__`, `__INGRESS_HOST__`, `__CLUSTER_ISSUER__`, `__STORAGE_CLASS__`. Copy the project to start a new environment. |
| `panservice-servizi/overlays/prod` | ⬜ untracked | RKE2: ClusterIP + Ingress `magikup.decisyon.com`, cert-manager `letsencrypt-prod`, default SC, **no** NetworkPolicy (hostNetwork ingress). |
| `vetri_speciali_gcp/overlays/prod` | ⬜ untracked | GKE: **internal LoadBalancer** (`service-lb.yaml`), no Ingress, default SC. |
| `testdialberto/overlays/test` | ⬜ untracked | Rancher/AWS: Ingress `magikup.decisyon.site` + `letsencrypt-prod`, StorageClass `auto-ebs-gp3`. |
| `skf-pume-dev/overlays/dev` | ⬜ untracked | AKS/Arc: Ingress class `nginx-internal`, namespace `d2w-test`. |
| `kubernetes/secret.yaml` | ⬜ untracked | Secret `magikup-secret` (Fernet key). **Out-of-band, gitignored** — generated by `scripts/create-secret.sh`, applied with `kubectl apply -f`, never part of a kustomization. |

> New environment: `cp -r kubernetes/template kubernetes/<project>`, fill the overlay placeholders, add the folder to `.gitignore`. Full steps in [`kubernetes/template/README.md`](kubernetes/template/README.md).
